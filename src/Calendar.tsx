...
import { AddEditResourcesDialog } from "./AddEditResources";
import { FreeFormResourceSource, IResource } from "./FreeformResourceSource";
import { weekOfYear } from "@fullcalendar/core/datelib/marker";
...
class ExtensionContent extends React.Component {
    ...
    selectedResourceTitle: string | undefined;
    ...
    constructor(props: {}) {
        ...
        this.resourceSource = new FreeFormResourceSource();
        ...
        this.commandBarItems = [
            ...
            },
            {
                ...
                id: "Resource",
                important: true,
                onActivate: this.onClickNewResource,
                text: "Resource"
            },
            {
                id: "today",
            ...
            // month view
            {
                ...
                onActivate: () => {
                    if (this.calendarComponentRef.current) {
                        this.getCalendarApi().changeView("dayGridMonth");
                    }
                },
                text: "Month"
            },
            // resource view
            {
                ...
                onActivate: () => {
                    if (this.calendarComponentRef.current) {
                        this.getCalendarApi().changeView("resourceTimeline");
                    }
                },
                text: "Resource"
            }
        ...
        return (
            <Page className="flex-grow flex-row">
                <div className="flex-column scroll-hidden calendar-area">
                    ...
                    <Observer display={this.displayCalendar} resourceSource={this.resourceSource.getResourcesObservable()}>
                        {(props: { display: boolean; resourceSource: IResource[] }) => {
                            return props.display ? (
                                <div className="calendar-component">
                                    <FullCalendar
                                        ...
                                        //add custom btn then add them to header
                                        customButtons={{
                                            newEvent: {
                                                text: "New Event",
                                                click: () => {
                                        ...
                                        header={{
                                            left: "",
                                            center: "title",
                                            right: "newEvent, newResource,   today, prev, next,    dayGridMonth, resourceTimeline"
                                        }}
                    ...
                    {(props: { dialog: Dialogs }) => {
                        return props.dialog === Dialogs.NewDaysOffDialog ? (
                            ...
                            <AddEditEventDialog
                                ...
                                resourcesSource={this.resourceSource}
                                onDismiss={this.onDialogDismiss}
                                start={this.selectedStartDate}
                                resourceId={this.selectedResourceId}
                                resourceTitle={this.selectedResourceTitle}
                            />
                        ) : null;
                    }}
                </Observer>
                <Observer dialog={this.openDialog}>
                    {(props: { dialog: Dialogs }) => {
                        return props.dialog === Dialogs.NewResourceDialog ? (
                            <AddEditResourcesDialog
                                calendarApi={this.getCalendarApi()}
                                eventSource={this.freeFormEventSource}
    ...
    private async initialize() {
        ...
        if (project) {
            ...
            this.resourceSource.initialize(selectedTeamId, this.dataManager);
    ...
    // Method to open the resource dialog
    private onClickNewResource = () => {
        this.openDialog.value = Dialogs.NewResourceDialog; // Set the dialog to show the new resource form
    };
    ...
    private onEventDrop = (arg: {
        ...
        const oldResourceId = arg.oldEvent.getResources()?.[0]?.id;
        const newResourceId = arg.event.getResources()?.[0]?.id;

        if (oldResourceId !== newResourceId) {
            console.log(`Resource changed from ${oldResourceId} to ${newResourceId}`);
        }

        // Update the event with the new resourceId if needed
        this.freeFormEventSource.updateEvent(
            ...
            newResourceId // Pass the new resourceId
        );

        // Ensure FullCalendar reflects the change
        arg.event.setProp("resourceId", newResourceId);
    };
    ...
    private onSelectCalendarDates = (arg: {
        ...
        this.selectedEndDate.setDate(arg.end.getDate());
        this.selectedStartDate = arg.start;

        // Store selected resource if available
        if (arg.resource) {
            this.selectedResourceId = arg.resource.id;
            this.selectedResourceTitle = arg.resource.title;

            // Log or handle the selected resource's title (or category)
            console.log(`Selected resource: ${this.selectedResourceTitle}`);
        } else {
            this.selectedResourceId = undefined;
            this.selectedResourceTitle = undefined;
        }
        ...
        if (currentView === "resourceTimelineDay" || currentView === "resourceTimelineWeek" || currentView === "resourceTimeline") {
            // For resource views, position the menu at the mouse click position
            const tempElement = document.createElement("div");
            tempElement.style.position = "absolute";
            tempElement.style.left = `${arg.jsEvent.clientX}px`;
            tempElement.style.top = `${arg.jsEvent.clientY}px`;
            document.body.appendChild(tempElement);

            // Store selected resource if available
            if (arg.resource) {
                console.log("Selected resource:", arg.resource.id, arg.resource.title);
            }

            this.anchorElement.value = tempElement;

            // Remove the temporary element when the menu is dismissed
            const originalDismiss = this.anchorElement.value;
            this.anchorElement.subscribe((newValue) => {
                if (newValue === undefined && originalDismiss === tempElement) {
                    document.body.removeChild(tempElement);
                }
            });
        } else {
            // For other views (month, day, week), use the original logic
    ...
    private onSelectTeam = async (event: React.SyntheticEvent<HTMLElement, Event>, item: IListBoxItem<{}>) => {
        ...
        this.resourceSource.initialize(newTeam.id, this.dataManager!);
