import { ExtensionDataCollection, IExtensionDataManager } from "azure-devops-extension-api";
...
import { formatDate, getMonthYearInRange, shiftToLocal, shiftToUTC } from "./TimeLib";
...
export class FreeFormEventsSource {
    ...
    public addEvent = (
        ...
        const event: ICalendarEvent = {
            ...
            title: title,
            resourceId: resourceId // Store resource ID if assigned
        };
            ...
            Object.keys(this.eventMap).forEach((id) => {
                ...
                if (Date.parse(event.startDate) && event.endDate && Date.parse(event.endDate)) {
                    ...
                    if ((calendarStart <= start && start <= calendarEnd) || (calendarStart <= end && end <= calendarEnd)) {
                        ...
                        // Add resourceId if exists to support resource views
                        if (event.resourceId) {
                            eventInput.resourceId = event.resourceId;
                        }
    ...
    public updateEvent = (
        ...
        if (resourceId) {
            oldEvent.resourceId = resourceId;
        }
